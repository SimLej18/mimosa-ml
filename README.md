...
# 4 tasks, 2 clusters, 1D inputs/outputs, 10 points/task
...
init_params = Parameters(
	...
	task_kernel=VarianceKernel(1.0) * SEKernel(length_scale=1.0),
	noise_kernel=WhiteNoiseKernel(noise=0.1),
)

key, subkey = jr.split(key)
dataset, grid, hyperprior, mixture, parameters, cluster_means, tasks = generate_data(
	subkey, dims, init_params, ModelConfig())

# Fit a model on the generated dataset, starting from the same parameters
model = BasicModel(jr.PRNGKey(1), n_clusters=dims.K, jitter=jnp.asarray(1e-5))
fitted_params, fitted_mixture = model.fit(dataset, mixture.proportions, parameters, n_iter=50)

# Predict the posterior distribution of every task, in every cluster
predictions = model.predict(dataset, fitted_mixture, fitted_params)

# TODO: plot `predictions` against `dataset` (see mimosa-ml/plot.py, not implemented yet)
```
