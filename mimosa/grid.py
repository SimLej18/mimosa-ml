...
class MultiOutputUnionGrid(MultiOutputGridBuilder):
	...
	def __call__(self, dataset: Dataset, config: ModelConfig) -> Grid:
		...
		point (NaN on every input dimension) maps to `mimosa.PAD_INDEX`, i.e. outside the
	...
	def _per_output_grid(self, inputs: Array, output_ids: Array, T: int) -> Grid:
		...
		starts = jnp.concatenate(
			[jnp.zeros((1,), dtype=int), jnp.cumsum(jnp.array([len(p) for p in block_points]))[:-1]]
		)
		...
		return Grid(points=points, output_ids=grid_output_ids, mappings=mappings, n_outputs=self.n_outputs)
