...
class LaplaceApproximator(eqx.Module):
	...
	@abstractmethod
	def link(
		self, total: Float[Array, "..."], count: Float[Array, "..."]
	...
	def wrap(self, dataset: Dataset) -> Dataset:
		...
		if self.interval == 0.0:
			...
			return Dataset(
				inputs=dataset.inputs, outputs=mean, known_output_noise=variance, output_ids=dataset.output_ids
			)
	...
	@staticmethod
	def _scatter(
		bin_ix: Int[Array, "R N"], observed: Bool[Array, "R N C"], values: Float[Array, "R N C"], R: int, n_kept: int
	...
	def _lattice(
		self, inputs: Float[Array, "R N I"], usable: Bool[Array, "R N"]
		...
		per_dim = jnp.clip(jnp.floor((clean - lo) / self.interval).astype(int), 0, jnp.asarray(n_bins) - 1)
		address = jnp.ravel_multi_index(tuple(per_dim[..., d] for d in range(len(n_bins))), n_bins, mode="clip")
		...
		acc = jax.ops.segment_sum(stat.reshape(-1, stat.shape[-1]), bin_ix.reshape(-1), num_segments=n_kept + 1)[
			:n_kept
		]
		return acc[:, :-1] / acc[:, -1:], bin_ix, n_kept

	def _masked_link(
		self, total: Float[Array, "T ON C"], count: Float[Array, "T ON C"]
